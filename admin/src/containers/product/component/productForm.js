...
class ProductForm extends Component {
    ...
    componentDidMount() {
        ...
        {
            const {receiveData: {productDetail:{_id, price, sale, content, statusSent,status,categoryId, path}}} = this.props;
            ...
            const title = this.props.receiveData.productDetail.title;
            this.setState({
                title:  title
            });
        }
    ...
    onHandleUpdateForm = () => {
        ...
        if(this.state.editorName.length !== 0 ) {
            ...
            if(typeof this.props.receiveData.productDetail.content !== "undefined") {
                contentEditor = this.props.receiveData.productDetail.content;
            }
        }
...
const mapDispatchToProps = (dispatch) => ({
    onProductionSubmitCreate: (data) => dispatch(productAction.create(data)),
    onUpdateProduction:(dataUpdate) => dispatch(productAction.update(dataUpdate)),
    toggleNewProduct: (state) => dispatch(productAction.toggleNewProduct(state)),
    onGetProducts: (userId) => dispatch(productAction.getByUserId(userId)),
    collapseTable: (state) => dispatch(productAction.collapseTable(state)),
